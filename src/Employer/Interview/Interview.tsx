...
function Interview() {
    ...
    useEffect(() => {
        ...
        const socket = new WebSocket(getEndpoint() + `?token=${localStorage.getItem('access-token')}&target=${routerStates.state && routerStates.state.targetid ? routerStates.state.targetid : user.user_id}`)
        ...
        socket.onmessage = async event => {
            ...
            if (recieved_data.user != user.user_id) {
                ...
                if (recieved_data.response === 'audio_muted') {
                    info("User muted their mic")
                }
                if (recieved_data.response == 'audio_unmuted') {
                    success("User unmuted their mic")
                }
    ...
    return (
        <div>
            <div className="main-container-videos">
                ...
                <div className="controls">
                    <p
                        className={VideoMuted ? "app-shadow btn-1 s-s muted" : "app-shadow btn-1 s-s"}
                        onClick={() => {
                    ...
                    <p
                        className={AudioMuted ? "app-shadow btn-1 s-s muted" : "app-shadow btn-1 s-s"}
                        onClick={() => {
                    ...
                    <p className="app-shadow btn-1 leave" onClick={() => { window.location.href = '/' }}>
                    ...
                    <p onClick={toggleScreenShare}>ShareScreen</p>
